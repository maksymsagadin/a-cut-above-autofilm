...
const FeaturedPostCard = ({ post }) => (
  <div className="relative h-96">
    ...
    <div className="flex flex-col rounded-lg p-6 items-end justify-end absolute w-full h-full">
        <p className="text-white text-shadow font-semibold text-sm p-1 rounded-t bg-yellow-600 bg-opacity-70">{moment(post.createdAt).format('MMM DD, YYYY')}</p>
        <p className="text-white text-shadow font-semibold text-sm p-1 rounded rounded-tr-none bg-yellow-600 bg-opacity-70">{post.title}</p>
    </div>
